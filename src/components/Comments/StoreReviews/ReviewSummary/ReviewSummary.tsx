...
function getAveraggeRating(comments: Comment[]) {
    const total = comments.reduce((prev, curComment) => prev + curComment.rating, 0);
...
function getRatingsPercentagesForEachRange(comments: Comment[], reviewsCount: number) {
    let zeroToTwo = 0,
        twoToFour = 0,
        fourToSix = 0,
        sixToEight = 0,
...
const ReviewSummary: React.FC<Props> = ({ comments }) => {
    ...
    const { eightToTen, sixToEight, fourToSix, twoToFour, zeroToTwo } =
        getRatingsPercentagesForEachRange(comments, reviewsCount);

    return (
        <section className="review-summary">
            <div className="review-heading">
                <div className="review-heading__rating-number">{averageRating}</div>
                <Rating
                    ...
                    readOnly
                />
                <p className="review-heading__rating-count">{reviewsCount} reviews</p>
            </div>
