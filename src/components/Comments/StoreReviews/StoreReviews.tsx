...
const StoreComment: React.FC<{ movie: Movie }> = ({ movie }) => {
    ...
    const alreadyHaveOne = userAlreadyReviewed(comments, user ? user.email : '');
    ...
    async function addReviewHandler(newRating: number, newComment: string) {
        if (!user) return;
        if (userHasReview) {
            alert('You alreay have a review for this movie!');
            return;
        }
    ...
    function editReviewHandler(commentId: string, newRating: number, newComment: string) {
