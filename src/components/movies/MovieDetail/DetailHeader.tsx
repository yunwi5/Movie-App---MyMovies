    ...
    const similarMovies = useMemo(
		() => {
			...
			return similarMoviesAvailable.slice(
				0,
				Math.min(similarMoviesAvailable.length, 7)
			);
		},
		[ movieId ]
	);
    ...
    return (
        ...
        <section className="movie-detail__top" onClick={backdropNavHandler}>
				...
				<div className="movie-detail__links">
					{isForUser && <button className="btn btn-favorite" onClick={onFavorite}>
						{!movie.isFavorite && <><i className="fa fa-star" ></i>Favorite</>}
						{movie.isFavorite && <><i className="fa fa-star-o" ></i>Unfavorite</>}
					</button> }
					{!isForUser && <button className="btn btn-add" onClick={() => onShowAddModal(true)}> 
						Collect
					</button>}
